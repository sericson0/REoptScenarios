...
#Simple tariff and only PV
m = Model(Optimizer_Type)
params = initialize_system(m, "../test/Inputs/monthly_rate.json")
grid_scenario_vals = initialize_grid_scenarios(m, "../test/Inputs/monthly_rate.json", params)
@objective(m, Min, params["system_cost"] + grid_scenario_vals.costs)
optimize!(m)

println("Optimized. Objective value is ", round(objective_value(m); digits=  2), ". should be 437169.27")
println("PV size is: ", round(value(m[:dv_pv_kw]); digits = 2), ". Should be 64.25")
